...
class Notification(object):
    ...
    def send_new_answer_comment(cls, answer_comment):
        ...
        # send the message
        # this notification will be triggered by a HTTP request, so url_for can be used as backup to generate the url
        ...
        html_body = render_template(
            ...
            compair_app_url_base=current_app.config.get( \
                'COMPAIR_APP_URL_BASE', \
                 url_for('route_app', _external=True, _anchor='')),
        )
    ...
    def send_answer_period_ending_soon(cls, course, assignment, student):
        ...
        html_body = render_template(
            ...
            course=course,
            assignment=assignment,
            compair_app_url_base=current_app.config.get('COMPAIR_APP_URL_BASE', ''),
        )
        text_body = render_template(
